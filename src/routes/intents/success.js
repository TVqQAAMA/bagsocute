...
async function getFileList() {
  const req = await fetch(
    `https://api.netlify.com/api/v1/sites/${process.env['NETLIFY_SITE']}/files`,
    {
      method: 'GET',
      headers: {
  ...
  for (let i = 0; i < response.length; i += 1) {
    hashList[response[i].path] = response[i].sha;
  }
...
async function deploy() {
  ...
  req = await fetch(
    `https://api.netlify.com/api/v1/sites/${process.env['NETLIFY_SITE']}/deploys`,
    {
      method: 'POST',
      headers: {
